...
const ManageAttendance = () => {
  ...
  useEffect(() => {
    if (selectedCourse) {
      axios
        .get(`http://localhost:5000/api/admin/attendance/${selectedCourse}`, {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        })
        .then((res) => {
  ...
  const handleCreate = (e) => {
    ...
    axios
      .post(
        `http://localhost:5000/api/admin/attendance/${selectedCourse}`,
        newAttendance,
        {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        }
  ...
  const handleUpdate = (id, updatedStatus) => {
    ...
    axios
      .put(
        `http://localhost:5000/api/admin/attendance/${id}`,
  ...
  const handleDelete = (id) => {
    axios
      .delete(`http://localhost:5000/api/admin/attendance/${id}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
      })
      .then(() => {
        setAttendanceRecords((prev) =>
